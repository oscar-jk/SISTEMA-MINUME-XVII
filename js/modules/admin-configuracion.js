...
async function pintarTolerancias(el) {
  const { data: filas } = await supabase.from('tolerancias_puntualidad').select('*').order('subsecretaria');
  el.innerHTML = `
    <h3 class="subtitulo" style="margin-top:0">Tolerancias de puntualidad</h3>
    <form class="formulario" data-form>
      <div class="formulario__fila">
        <label class="campo"><span>Subsecretaría</span><input name="subsecretaria" required /></label>
      ...
      <button type="submit" class="boton boton--secundario">${icono('mas', { tamano: 16 })} Añadir</button>
    </form>
    <div data-lista></div>
  `;
  el.querySelector('[data-form]').addEventListener('submit', async (e) => {
    e.preventDefault();
    const datos = datosFormulario(e.target);
    datos.tolerancia_minutos = Number(datos.tolerancia_minutos);
    const { error } = await supabase.from('tolerancias_puntualidad').upsert(datos, { onConflict: 'subsecretaria' });
